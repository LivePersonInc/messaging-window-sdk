...
function windowKit(options) {
	...
	this.clearConversation = function() {
		...
		_this.console('Conversation has been cleared from browser history.');
	};
	...
	this.connect = function(JWT) {
		if (!_this.options.forever) {
			_this.clearConversation();
		}
		if (JWT !== undefined) {
			_this.options.authenticated = true;
			window.preFillJWT = JWT;
		}
